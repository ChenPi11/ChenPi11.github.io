...
func MarkdownIt(markdownText string) string {
	...
	if err != nil {
		color.Error.Print(gotext.Get("Error creating temporary file: %v\n", err))
		os.Exit(1)
	}
	defer os.Remove(tempInputFile.Name())
	_, err = tempInputFile.WriteString(markdownText)
	if err != nil {
		color.Error.Print(gotext.Get("Error writing to temporary file: %v\n", err))
		os.Exit(1)
	}
	tempInputFile.Close()
	...
	if GetVerbose() {
		color.Notice.Print(gotext.Get("Executing command %s %s ...\n", markdownItExecutable[0], strings.Join(args, " ")))
	}

	cmd := exec.Command(markdownItExecutable[0], args...)
	output, err := cmd.Output()
	if err != nil {
		color.Error.Print(gotext.Get("Error while executing command: %v\n", err))
		os.Exit(1)
	}
